...
use meilisearch_types::milli::documents::{
    obkv_to_object, DocumentsBatchReader, PrimaryKey,
};
...
use crate::processing::{
    AtomicBatchStep, AtomicDocumentStep, AtomicTaskStep, AtomicUpdateFileStep,
    CreateIndexProgress, DeleteIndexProgress, DocumentDeletionProgress,
    DocumentEditionProgress, DocumentOperationProgress, DumpCreationProgress,
    InnerSwappingTwoIndexes, SettingsProgress, SnapshotCreationProgress,
    SwappingTheIndexes, TaskCancelationProgress, TaskDeletionProgress,
...
pub(crate) enum IndexOperation {
    ...
    DocumentEdition { index_uid: String, task: Task },
    DocumentDeletion { index_uid: String, tasks: Vec<Task> },
    DocumentClear { index_uid: String, tasks: Vec<Task> },
    Settings {
        index_uid: String,
            ...
            Batch::IndexOperation { op, .. } => {
                match op {
                    ...
                    IndexOperation::DocumentEdition { task, .. } => {
                        RoaringBitmap::from_sorted_iter(std::iter::once(task.uid))
                            .unwrap()
                    }
                    IndexOperation::DocumentClearAndSetting {
                        ...
                        RoaringBitmap::from_iter(
                            tasks.iter().chain(other).map(|task| task.uid),
                        )
                    }
                }
            }
...
impl IndexScheduler {
    pub(crate) fn create_next_batch_index(
        ...
        match batch {
                ...
                Ok(
                    Some(Batch::IndexOperation {
                        op: IndexOperation::DocumentClear {
                            tasks: self
                                .get_existing_tasks_for_processing_batch(
                                    rtxn,
                                    current_batch,
                                    ids,
                                )?,
                ...
                for task in tasks.iter() {
                    match task.kind {
                        ...
                        KindWithContent::DocumentDeletion { ref documents_ids, .. } => {
                            operations
                                .push(DocumentOperation::Delete(documents_ids.clone()));
                        }
                        _ => unreachable!(),
                    }
                }
            ...
            BatchKind::IndexDeletion { ids } => {
                Ok(
                    Some(Batch::IndexDeletion {
                        index_uid,
    ...
    pub(crate) fn create_next_batch(
        ...
        #[cfg(test)] self.maybe_fail(crate::tests::FailureLocation::InsideCreateBatch)?;
        ...
        let index_name = if let Some(&index_name) = task.indexes().first() {
            ...
            assert!(
                matches!(& task.kind, KindWithContent::IndexSwap { swaps } if swaps
                .is_empty())
            );
            current_batch.processing(Some(&mut task));
        ...
        let tasks_limit = if self.autobatching_enabled {
            self.max_number_of_batched_tasks
        } else {
            1
        };
        ...
        if let Some((batchkind, create_index)) = autobatcher::autobatch(
            enqueued,
    ...
    pub(crate) fn process_batch(
        ...
        match batch {
            Batch::TaskCancelation { mut task } => {
                let matched_tasks = if let KindWithContent::TaskCancelation {
                    tasks,
                    query: _,
                } = &task.kind
                {
                ...
                let mut canceled_tasks = self
                    .cancel_matched_tasks(
                        &rtxn,
                        task.uid,
                        current_batch,
                        matched_tasks,
            ...
            Batch::TaskDeletions(mut tasks) => {
                ...
                let mut deleted_tasks = self
                    .delete_matched_tasks(&mut wtxn, &matched_tasks, &progress)?;
                wtxn.commit()?;
                for task in tasks.iter_mut() {
                    task.status = Status::Succeeded;
                    let KindWithContent::TaskDeletion { tasks, query: _ } = &task.kind
                    else { unreachable!() };
            ...
            Batch::SnapshotCreation(mut tasks) => {
                progress
                    .update_progress(SnapshotCreationProgress::StartTheSnapshotCreation);
                ...
                progress
                    .update_progress(
                        SnapshotCreationProgress::SnapshotTheIndexScheduler,
                    );
                ...
                progress
                    .update_progress(SnapshotCreationProgress::SnapshotTheUpdateFiles);
                let enqueued = self.get_status(&rtxn, Status::Enqueued)?;
                let (atomic, update_file_progress) = AtomicUpdateFileStep::new(
                    enqueued.len() as u32,
                );
                ...
                for (i, result) in index_mapping.iter(&rtxn)?.enumerate() {
                    ...
                    progress
                        .update_progress(
                            VariableNameStep::new(name, i as u32, nb_indexes),
                        );
                ...
                base_path.pop();
                let db_name = base_path
                    .file_name()
                    .and_then(OsStr::to_str)
                    .unwrap_or("data.ms");
                let snapshot_path = self
                    .snapshots_path
                    .join(format!("{}.snapshot", db_name));
                let temp_snapshot_file = tempfile::NamedTempFile::new_in(
                    &self.snapshots_path,
                )?;
                compression::to_tar_gz(
            ...
            Batch::Dump(mut task) => {
                ...
                let (keys, instance_uid) = if let KindWithContent::DumpCreation {
                    keys,
                    instance_uid,
                } = &task.kind
                {
                ...
                let (atomic, update_task_progress) = AtomicTaskStep::new(
                    self.all_tasks.len(&rtxn)? as u32,
                );
                ...
                for ret in self.all_tasks.iter(&rtxn)? {
                    ...
                    if let Some(content_file) = content_file {
                        ...
                        if status == Status::Enqueued {
                            ...
                            let (mut cursor, documents_batch_index) = reader
                ...
                self.index_mapper
                    .try_for_each_index(
                        ...
                        |uid, index| -> Result<()> {
                            progress
                                .update_progress(
                                    VariableNameStep::new(uid.to_string(), count, nb_indexes),
                                );
                            count += 1;
                            let rtxn = index.read_txn()?;
                            ...
                            let all_fields: Vec<_> = fields_ids_map
                                .iter()
                                .map(|(id, _)| id)
                                .collect();
                            ...
                            let nb_documents = index
                                ...
                                .map_err(|e| Error::from_milli(e, Some(uid.to_string())))?
                                as u32;
                            let (atomic, update_document_progress) = AtomicDocumentStep::new(
                                nb_documents,
                            );
                            ...
                            for ret in documents {
                                ...
                                let (id, doc) = ret
                                    .map_err(|e| Error::from_milli(e, Some(uid.to_string())))?;
                                let mut document = milli::obkv_to_json(
                                        &all_fields,
                                        &fields_ids_map,
                                        doc,
                                    )
                                ...
                                'inject_vectors: {
                                    ...
                                    let serde_json::Value::Object(vectors) = vectors else {
                                        let user_err = milli::Error::UserError(milli::UserError::InvalidVectorsMapType {
                                            document_id: {
                                        ...
                                        return Err(
                                            Error::from_milli(user_err, Some(uid.to_string())),
                                        );
                                    };
                ...
                progress
                    .update_progress(DumpCreationProgress::DumpTheExperimentalFeatures);
                let features = self.features().runtime_features();
                dump.create_experimental_features(features)?;
                let dump_uid = started_at
                    .format(
                        format_description!(
                ...
                task.status = Status::Succeeded;
                task.details = Some(Details::Dump {
            ...
            Batch::IndexOperation { op, must_create_index } => {
                ...
                self.index_mapper
                    .set_currently_updating_index(
                        Some((index_uid.clone(), index.clone())),
                    );
                let mut index_wtxn = index.write_txn()?;
                let tasks = self
                    .apply_index_operation(&mut index_wtxn, &index, op, progress)?;
                {
                    let span = tracing::trace_span!(
                        target : "indexing::scheduler", "commit"
                    );
            ...
            Batch::IndexSwap { mut task } => {
                progress
                    .update_progress(SwappingTheIndexes::EnsuringCorrectnessOfTheSwap);
                let mut wtxn = self.env.write_txn()?;
                ...
                if !not_found_indexes.is_empty() {
                    if not_found_indexes.len() == 1 {
                        return Err(
                            Error::SwapIndexNotFound(
                                not_found_indexes.into_iter().next().unwrap().clone(),
                            ),
                        );
                    } else {
                        return Err(
                            Error::SwapIndexesNotFound(
                ...
                for (step, swap) in swaps.iter().enumerate() {
                    progress
                        .update_progress(
                            VariableNameStep::new(
                                format!(
                                    "swapping index {} and {}", swap.indexes.0, swap.indexes.1
                                ),
                                step as u32,
                                swaps.len() as u32,
                            ),
        ...
        match operation {
            ...
            IndexOperation::DocumentOperation {
                ...
                for (stats, task) in operation_stats.into_iter().zip(&mut tasks) {
                    ...
                    task.details = match task.details {
                        Some(
                            Details::DocumentAdditionOrUpdate { received_documents, .. },
                        ) => {
                ...
                if tasks.iter().any(|res| res.error.is_none()) {
                    ...
                    let addition = DocumentAdditionResult {
                        ...
                        number_of_documents: index
                            ...
                            .map_err(|err| Error::from_milli(
                                err,
                                Some(index_uid.clone()),
                            ))?,
                    };
                    tracing::info!(
                        indexing_result = ? addition, processed_in = ?
                        started_processing_at.elapsed(), "document indexing done"
                    );
                }
                Ok(tasks)
            }
            IndexOperation::DocumentEdition { index_uid, mut task } => {
                ...
                let candidates = match filter.as_ref().map(Filter::from_json) {
                    ...
                    Some(Err(e)) => {
                        return Err(Error::from_milli(e, Some(index_uid.clone())));
                    }
                };
                let (original_filter, context, function) = if let Some(
                    Details::DocumentEdition { original_filter, context, function, .. },
                ) = task.details
                {
                ...
                let primary_key = PrimaryKey::new_or_insert(
                        primary_key,
                        &mut new_fields_ids_map,
                    )
                    .map_err(|err| Error::from_milli(
                ...
                if task.error.is_none() {
                    ...
                    progress
                        .update_progress(DocumentEditionProgress::ComputingTheChanges);
            ...
            IndexOperation::DocumentDeletion { mut tasks, index_uid } => {
                    ...
                    match &task.kind {
                        KindWithContent::DocumentDeletion {
                            ...
                            for id in documents_ids {
                                if let Some(id) = external_documents_ids
                                    .get(index_wtxn, id)?
                                {
                                    to_delete.insert(id);
                                }
                ...
                if !tasks.iter().all(|res| res.error.is_some()) {
                    ...
                    let document_changes = indexer
                        .into_changes(&indexer_alloc, primary_key);
                    let embedders = index
                        .embedding_configs(index_wtxn)
                        .map_err(|err| Error::from_milli(err, Some(index_uid.clone())))?;
                    let embedders = self.embedders(index_uid.clone(), embedders)?;
            ...
            IndexOperation::Settings { index_uid, settings, mut tasks } => {
                progress
                    .update_progress(SettingsProgress::RetrievingAndMergingTheSettings);
                let indexer_config = self.index_mapper.indexer_config();
                ...
                for (task, (_, settings)) in tasks.iter_mut().zip(settings) {
                    ...
                    task.details = Some(Details::SettingsUpdate {
            ...
            IndexOperation::DocumentClearAndSetting {
                ...
                let mut import_tasks = self
                    .apply_index_operation(
                        index_wtxn,
                        index,
                        IndexOperation::DocumentClear {
                            index_uid: index_uid.clone(),
    ...
    fn delete_matched_tasks(
        ...
        let (atomic_progress, task_progress) = AtomicTaskStep::new(
            (affected_indexes.len() + affected_statuses.len() + affected_kinds.len())
                as u32,
        );
        progress.update_progress(task_progress);
        ...
        let (atomic_progress, batch_progress) = AtomicBatchStep::new(
            affected_batches.len() as u32,
        );
    ...
    fn cancel_matched_tasks(
        ...
        let (task_progress, progress_obj) = AtomicTaskStep::new(
            tasks_to_cancel.len() as u32,
        );
