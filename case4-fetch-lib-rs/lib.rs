...
pub struct Options {
    ...
    pub request_builder_hook: Option<
        fn(&mut http::Request<ReqBody>) -> Result<(), JsErrorBox>,
    >,
...
deno_core::extension!(
    deno_fetch, deps = [deno_webidl, deno_web], ops = [op_fetch, op_fetch_send,
    op_utf8_to_byte_string, op_fetch_custom_client, op_fetch_promise_is_settled,],
    lazy_loaded_js = ["20_headers.js", "21_formdata.js", "22_body.js",
    "22_http_client.js", "23_request.js", "23_response.js", "26_fetch.js",
    "27_eventsource.js"], options = { options : Options, }, state = | state, options | {
...
impl From<deno_fs::FsError> for FetchError {
    fn from(value: deno_fs::FsError) -> Self {
        match value {
            ...
            deno_fs::FsError::PermissionCheck(err) => FetchError::PermissionCheck(err),
...
pub type CancelableResponseFuture = Pin<
    Box<dyn Future<Output = CancelableResponseResult>>,
>;
...
pub fn op_fetch(
    ...
    let (request_rid, cancel_handle_rid) = match scheme {
        "file" => {
            ...
            let (future, maybe_cancel_handle) = file_fetch_handler
                .fetch_file(state, &url);
        ...
        "http" | "https" => {
            ...
            *request.uri_mut() = uri.clone();
            ...
            let fut = async move {
                client.send(request).map_err(Into::into).or_cancel(cancel_handle_).await
            };
            let request_rid = state
                .resource_table
                .add(FetchRequestResource {
...
type CancelableResponseResult = Result<
...
impl Resource for FetchResponseResource {
    ...
    fn read(self: Rc<Self>, limit: usize) -> AsyncResult<BufView> {
        Box::pin(async move {
            let mut reader = RcRef::map(&self, |r| &r.response_reader)
                .borrow_mut()
                .await;
            let body = loop {
                match &mut *reader {
            ...
            let fut = async move {
                ...
                loop {
                    match reader.as_mut().peek_mut().await {
                        ...
                        Some(_) => {
                            match reader.as_mut().next().await.unwrap() {
                                Ok(chunk) => assert!(chunk.is_empty()),
                                Err(err) => {
                                    break Err(JsErrorBox::type_error(err.to_string()));
                                }
                            }
                        }
            ...
            fut.try_or_cancel(cancel_handle).await.map_err(JsErrorBox::from_err)
        })
    }
...
pub fn create_http_client(
    ...
    let mut http_connector = HttpConnector::new_with_resolver(
    ...
    if let Some(pool_idle_timeout) = options.pool_idle_timeout {
        builder
            .pool_idle_timeout(pool_idle_timeout.map(std::time::Duration::from_millis));
    }
...
where
    S: Service<http::Request<ReqBody>, Response = http::Response<Incoming>> + 'static,
...
fn decompress_response(
    ...
    let is_empty = resp
        ...
        .and_then(|v| v.parse::<u64>().ok()) == Some(0);
    if is_empty {
        return resp.map(box_raw_body);
    }
    match resp.headers().get(CONTENT_ENCODING).and_then(content_encoding_decode_kind) {
    ...
    R: tokio::io::AsyncRead + Send + Sync + 'static,
{
    let stream = ReaderStream::new(reader)
        .map(|result| {
            result.map(Frame::data).map_err(|err| JsErrorBox::generic(err.to_string()))
        });
    BoxBody::new(StreamBody::new(stream))
}
fn box_raw_body(body: Incoming) -> ResBody {
    body.map_err(|err| JsErrorBox::generic(err.to_string())).boxed()
}
#[derive(Debug, thiserror::Error, deno_error::JsError)]
...
>;
#[allow(clippy::declare_interior_mutable_const, reason = "clippy is wrong here")]
...
impl std::fmt::Display for ClientSendError {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        ...
        match self.http_info() {
            Some(http_info) => {
                write!(
                    f, "error sending request from {src} for {uri} ({dst}): {detail}",
                    src = http_info.local_addr(), uri = self.uri, dst = http_info
                    .remote_addr(), detail = detail,
                )
            }
            None => {
                write!(
                    f, "error sending request for url ({uri}): {detail}", uri = self.uri,
                    detail = detail,
                )
            }
        }
...
impl Client {
    pub fn inject_common_headers(&self, req: &mut impl CommonRequest) {
        req.headers_mut().entry(USER_AGENT).or_insert_with(|| self.user_agent.clone());
...
#[derive(Clone, Debug)]
struct Retried;
impl<ResBody, E> retry::Policy<http::Request<ReqBody>, http::Response<ResBody>, E>
for FetchRetry
where
...
fn is_error_retryable(err: &(dyn std::error::Error + 'static)) -> bool {
    if let Some(err) = find_source::<h2::Error>(err) {
        if err.is_go_away() && err.is_remote()
            && err.reason() == Some(h2::Reason::NO_ERROR)
        {
            return true;
        }
        if err.is_reset() && err.is_remote()
            && err.reason() == Some(h2::Reason::REFUSED_STREAM)
        {
            return true;
        }
    }
    if let Some(err) = find_source::<hyper::Error>(err) && err.is_incomplete_message() {
        return true;
    }
