...
pub async fn do_get_snapshot(
    ...
    let collection_pass = access
        .check_collection_access(collection_name, AccessRequirements::new().whole())?;
...
async fn list_snapshots(
    ...
    let response = do_list_snapshots(dispatcher.toc(&access), access, &collection_name)
        .await;
...
async fn create_snapshot(
    ...
    let collection_name = path.into_inner();
    helpers::time_or_accept_with_handle(
            ...
            async move {
                ...
                if let Some(checksum) = &params.checksum {
                    ...
                    if !hashes_equal(snapshot_checksum.as_str(), checksum.as_str()) {
                        return Err(
                            StorageError::checksum_mismatch(snapshot_checksum, checksum),
                        );
                    }
                }
...
async fn get_snapshot(
    ...
    do_get_snapshot(dispatcher.toc(&access), access, &collection_name, &snapshot_name)
        .await
}
#[get("/snapshots")]
...
async fn create_full_snapshot(
    ...
    helpers::time_or_accept_with_handle(
            params.wait.unwrap_or(true),
            async move { do_create_full_snapshot(dispatcher.get_ref(), access) },
        )
        .await
}
...
async fn delete_full_snapshot(
    ...
    helpers::time_or_accept_with_handle(
            ...
            async move {
                let snapshot_name = path.into_inner();
                do_delete_full_snapshot(dispatcher.get_ref(), access, &snapshot_name)
                    .await
            },
        )
        .await
...
async fn upload_shard_snapshot(
    ...
    let (collection, shard) = path.into_inner();
    let SnapshotUploadingParam { wait, priority, checksum } = query.into_inner();
    let future = cancel::future::spawn_cancel_on_drop(move |cancel| async move {
            ...
            let collection = cancel::future::cancel_on_token(cancel.clone(), future)
                .await??;
...
async fn download_shard_snapshot(
    ...
    let collection_pass = access
        .check_collection_access(&collection, AccessRequirements::new().whole())?;
    let collection = dispatcher.toc(&access).get_collection(&collection_pass).await?;
