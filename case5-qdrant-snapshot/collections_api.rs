...
use storage::content_manager::collection_meta_ops::{
    ChangeAliasesOperation, CollectionMetaOperations, CreateCollection,
...
fn get_collections(
    ...
    helpers::time(async move {
        do_list_collections(dispatcher.toc(&access), access).await
    })
}
#[get("/aliases")]
...
async fn get_collection(
    ...
    let timing = Instant::now();
    let response = do_get_collection(
...
async fn get_collection_existence(
    ...
    let timing = Instant::now();
    let response = do_collection_exists(
            dispatcher.toc(&access),
            access,
            &collection.name,
        )
...
async fn get_collection_aliases(
    ...
    let timing = Instant::now();
    let response = do_list_collection_aliases(
            dispatcher.toc(&access),
            access,
            &collection.name,
        )
