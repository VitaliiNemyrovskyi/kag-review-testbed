...
pub fn router() -> Router<SharedState> {
    Router::new()
        ...
        .route("/:repo_key/simple/:project", get(simple_project))
        .route("/:repo_key/simple/:project/:filename", get(download_or_metadata))
}
...
async fn simple_root(
    ...
    let raw_names: Vec<String> = sqlx::query_scalar!(
        ...
        .await
        .map_err(map_db_err)?;
    let mut merged: std::collections::BTreeSet<String> = raw_names
        .iter()
        .map(|n| normalize_pep503(n))
        .collect();
    if repo.repo_type == RepositoryType::Remote {
        if let Some(names) = fetch_remote_simple_root(
                &state,
                &repo.key,
                repo.id,
                &repo.upstream_url,
...
async fn fetch_remote_simple_root(
    ...
    if content.len() > MAX_SIMPLE_ROOT_BODY_BYTES {
        warn!(
            repo_key = % repo_key, upstream = % effective_upstream, body_bytes = content
            .len(), cap_bytes = MAX_SIMPLE_ROOT_BODY_BYTES,
...
fn build_simple_root_response(
    ...
    if accept.contains("application/vnd.pypi.simple.v1+json") {
        let json = serde_json::json!(
            { "meta" : { "api-version" : "1.2" }, "projects" : packages.iter().map(| p |
            { serde_json::json!({ "name" : p }) }).collect::< Vec < _ >> () }
        );
        return Ok(
            Response::builder()
...
async fn simple_project(
    ...
    if simple_artifacts.is_empty() {
        ...
        if repo.repo_type == RepositoryType::Virtual {
            ...
            let isolate = proxy_helpers::pypi_virtual_isolates_name(
                    &state.db,
                    repo.id,
                    &normalized,
                )
        ...
        "#,
            ...
            let local_member_ids: Vec<uuid::Uuid> = members
                ...
                .filter(|m| {
                    matches!(
                        m.repo_type, RepositoryType::Local | RepositoryType::Staging
                    )
                })
                .map(|m| m.id)
                .collect();
            let tracks = pypi_project_tracks_for(
                    &state.db,
                    &local_member_ids,
                    &normalized,
                )
            ...
            match (local_artifacts.is_empty(), remote_response) {
                ...
                (_, Some((content, content_type))) => {
                    ...
                    let body = if ct.contains("text/html") {
                        let html = String::from_utf8_lossy(&content);
                        let rewritten = rewrite_upstream_urls(
                            &html,
                            &repo_key,
                            &project,
                        );
...
fn build_simple_project_response(
    ...
    if accept.contains("application/vnd.pypi.simple.v1+json") {
        let files: Vec<serde_json::Value> = artifacts
            ...
            .map(|a| {
                ...
                let mut file = serde_json::json!(
                    { "filename" : filename, "url" : format!("/pypi/{}/simple/{}/{}",
                    repo_key, normalized, filename), "hashes" : { "sha256" : & a
                    .checksum_sha256 }, "size" : a.size_bytes, }
                );
                if let Some(rp) = requires_python {
                    file["requires-python"] = serde_json::Value::String(rp);
                }
                if let Some(ut) = a.upload_time {
                    file["upload-time"] = serde_json::Value::String(
                        ut.format("%Y-%m-%dT%H:%M:%SZ").to_string(),
                    );
                }
                file
            })
        ...
        if !tracks.is_empty() {
            meta["tracks"] = serde_json::Value::Array(
                tracks.iter().map(|t| serde_json::Value::String(t.clone())).collect(),
            );
        }
        let json = serde_json::json!(
            { "meta" : meta, "name" : normalized, "versions" : versions, "files" : files,
            }
        );
        return Ok(
            Response::builder()
    ...
    for t in tracks {
        html.push_str(
            &format!("<meta name=\"pypi:tracks\" content=\"{}\"/>\n", html_escape(t)),
        );
    }
    ...
    for a in artifacts {
        let filename = a.path.rsplit('/').next().unwrap_or(&a.path);
        let url = format!(
            "/pypi/{}/simple/{}/{}#sha256={}", repo_key, normalized, filename, a
            .checksum_sha256
        );
        let requires_python = a
            .metadata
        ...
        let ut_attr = a
            .upload_time
            .map(|ut| {
                format!(" data-upload-time=\"{}\"", ut.format("%Y-%m-%dT%H:%M:%SZ"))
            })
            .unwrap_or_default();
        html.push_str(
...
fn merge_local_into_remote_simple_html(
    ...
    static ANCHOR_FILENAME: Lazy<Regex> = Lazy::new(|| {
        Regex::new(r"(?s)<a\s[^>]*>([^<]+)</a>").unwrap()
    });
...
async fn serve_file(
    ...
    let artifact = sqlx::query!(
        ...
        LIMIT 1
        "#,
        repo.id, super::escape_filename_for_like(filename)
    )
            ...
            if repo.repo_type == RepositoryType::Virtual {
                ...
                for member in &members {
                        ...
                        .await
                    {
                        ...
                        Err(e) => {
                            debug!(
                                member_key = % member.key, error = % e.status(),
                                "local fetch failed for virtual member"
                            );
                        }
                    }
                    if member.repo_type == RepositoryType::Remote
                        && !suppress_remote_members
                    {
                        if let (Some(ref upstream_url), Some(ref proxy)) = (
                            &member.upstream_url,
                            &state.proxy_service,
                            ...
                            let normalized = PypiHandler::normalize_name(project);
                            let local_cache_path = format!(
                                "simple/{}/{}", normalized, filename
                            );
                            if let Some(redirect) = pypi_proxy_cache_redirect(
                                    state,
                                    proxy,
...
async fn resolve_pypi_remote_fetch_target(
    ...
    let normalized = PypiHandler::normalize_name(project);
    let (effective_upstream, index_path) = pypi_upstream_url_and_path(
        upstream_url,
        &format!("{}/", normalized),
    );
    ...
    let file_url = find_upstream_url_for_file(
        &index_html,
        filename,
        Some(&full_index_url),
    );
    ...
    if let Some(ref url) = file_url {
        if let Err(e) = validate_outbound_url(url, "PyPI upstream file URL") {
            ...
            return Err(
                AppError::Validation(
                        format!("Upstream index contains a disallowed URL: {}", e),
                    )
                    .into_response(),
            );
        }
    ...
    let (fetch_base, fetch_path) = match file_url
        .as_deref()
        .and_then(split_url_base_and_path)
    {
        Some(pair) => pair,
        None => fallback(),
    };
...
fn build_streaming_file_response(
    ...
    builder
        .body(
            Body::from_stream(
                result.body.map(|r| r.map_err(|e| std::io::Error::other(e.to_string()))),
            ),
        )
        .unwrap()
}
async fn serve_metadata(
    ...
    let artifact = sqlx::query!(
        ...
        LIMIT 1
        "#,
        repo_id, super::escape_filename_for_like(filename)
    )
        .fetch_optional(db)
        .await
        .map_err(map_db_err)?
    ...
    let content = storage.get(&artifact.storage_key).await.map_err(map_storage_err)?;
    ...
    match metadata_text {
        ...
        None => {
            Err(AppError::NotFound("Metadata not available".to_string()).into_response())
        }
    }
}
...
async fn upload(
            ...
            ":action" => {
                action = Some(
                    field
                        ...
                        .await
                        .map_err(|e| {
                            AppError::Validation(format!("Invalid field: {}", e))
                                .into_response()
                        })?,
                );
            }
                            ...
                            metadata_fields
                                .insert(
                                    ...
                                    serde_json::Value::Array(
                                        vec![existing.clone(), serde_json::Value::String(text),],
                                    ),
                                );
                        }
                    } else {
    ...
    if action != "file_upload" {
        return Err(
            AppError::Validation(format!("Unsupported action: {}", action))
                .into_response(),
        );
    }
    let pkg_name = pkg_name
        .ok_or_else(|| {
            AppError::Validation("Missing 'name' field".to_string()).into_response()
        })?;
    ...
    let filename = file_name
        .ok_or_else(|| {
            AppError::Validation("Missing filename in content field".to_string())
                .into_response()
        })?;
    ...
    if let Some(ref expected) = sha256_digest {
        if !expected.is_empty() && expected != &computed_sha256 {
            return Err(
                AppError::Validation(
                        format!(
                            "SHA256 mismatch: expected {} got {}", expected,
                            computed_sha256
                        ),
                    )
                    .into_response(),
    ...
    let existing = sqlx::query_scalar!(
        ...
        repo.id, format!("{}/{}/{}", normalized, pkg_version, filename)
    )
        .fetch_optional(&state.db)
        .await
        .map_err(map_db_err)?;
    if existing.is_some() {
        return Err(
            AppError::Conflict("File already exists".to_string()).into_response(),
        );
    }
    let mut pkg_metadata = serde_json::json!(
        { "name" : & pkg_name, "normalized_name" : & normalized, "version" : &
        pkg_version, "filename" : & filename, }
    );
    if let Some(rp) = &requires_python {
        pkg_metadata["pkg_info"] = serde_json::json!({ "requires_python" : rp, });
    }
    ...
    artifact_service
        ...
        .map_err(|e| e.into_response())?;
    let _ = sqlx::query!(
        "UPDATE repositories SET updated_at = NOW() WHERE id = $1", repo.id,
    )
        .execute(&state.db)
        .await;
    {
        let pkg_svc = crate::services::package_service::PackageService::new(
            state.db.clone(),
        );
        pkg_svc
            .try_create_or_update_from_artifact(
                ...
                summary.as_deref(),
                Some(
                    build_pypi_package_catalog_metadata(
                        &filename,
    ...
    info!(
        "PyPI upload: {} {} ({}) to repo {}", pkg_name, pkg_version, filename, repo_key
    );
...
fn build_pypi_package_catalog_metadata(
    ...
    let mut metadata = serde_json::json!({ "format" : "pypi", "filename" : filename, });
...
static METADATA_ATTR_RE: Lazy<Regex> = Lazy::new(|| {
...
mod tests {
    ...
    fn test_pypi_upstream_keeps_devpi_path() {
        let (url, path) = pypi_upstream_url_and_path(
            "https://devpi.example.com/root/pypi",
            "numpy/",
        );
    ...
    fn test_should_enqueue_pypi_sync_tasks_skips_peer_replication() {
        assert!(! should_enqueue_pypi_sync_tasks(& headers_with_replication("true")));
    }
    #[test]
    ...
    fn test_html_escape_apostrophe() {
        ...
        assert_eq!(html_escape("' onload='alert(1)"), "&#39; onload=&#39;alert(1)");
    }
    #[test]
    ...
    fn test_rewrite_multiple_links() {
        ...
        assert!(
            result
            .contains(r#"href="/pypi/my-pypi/simple/numpy/numpy-1.3.0.tar.gz#sha256=aaa""#)
        );
        assert!(
            result
            .contains(r#"href="/pypi/my-pypi/simple/numpy/numpy-1.4.0-cp39-cp39-manylinux1_x86_64.whl#sha256=bbb""#)
        );
    }
    #[test]
    fn test_rewrite_normalizes_project_name() {
        ...
        assert!(
            result
            .contains(r#"href="/pypi/pypi-remote/simple/my-package/My_Package-1.0.tar.gz#sha256=abc""#)
        );
    }
    #[test]
...
"#;
        ...
        assert!(
            result
            .contains(r#"href="/pypi/pypi-public/simple/numpy/numpy-2.0.0-cp312-cp312-manylinux_2_17_x86_64.manylinux2014_x86_64.whl#sha256=bbb222""#)
        );
    ...
    fn test_rewrite_mixed_absolute_and_relative() {
        ...
        let result = rewrite_upstream_urls(html, "repo", "pkg");
        assert!(
            result.contains(r#"href="/pypi/repo/simple/pkg/pkg-1.0.tar.gz#sha256=aaa""#)
        );
        assert!(
            result.contains(r#"href="/pypi/repo/simple/pkg/pkg-2.0.tar.gz#sha256=bbb""#)
        );
    }
    #[test]
    fn test_rewrite_url_with_deep_path() {
        ...
        assert!(
            result
            .contains(r#"href="/pypi/repo/simple/numpy/numpy-1.3.0.tar.gz#sha256=abc""#)
        );
    }
    #[test]
    fn test_rewrite_preserves_md5_fragment() {
        let html = r#"<a href="https://example.com/pkg-1.0.tar.gz#md5=deadbeef">pkg-1.0.tar.gz</a>"#;
        let result = rewrite_upstream_urls(html, "repo", "pkg");
        assert!(
            result
            .contains(r#"href="/pypi/repo/simple/pkg/pkg-1.0.tar.gz#md5=deadbeef""#)
        );
    }
    #[test]
    ...
    fn test_rewrite_local_upstream_with_data_attr() {
        ...
        assert!(
            result
            .contains(r#"href="/pypi/my-remote/simple/numpy/numpy-2.0.0-cp312-cp312-manylinux_2_17_x86_64.whl#sha256=bbb""#)
        );
    ...
    fn test_rewrite_mixed_absolute_and_local_relative() {
        ...
        assert!(
            result
            .contains(r#"href="/pypi/remote/simple/numpy/numpy-1.3.0.tar.gz#sha256=aaa""#)
        );
        assert!(
            result
            .contains(r#"href="/pypi/remote/simple/numpy/numpy-2.0.0.tar.gz#sha256=bbb""#)
        );
        assert!(
            result
            .contains(r#"href="/pypi/remote/simple/numpy/numpy-3.0.0.tar.gz#sha256=ccc""#)
        );
    }
    #[test]
...
"#;
        ...
        assert!(
            result
            .contains(r#"href="/pypi/remote-pypi/simple/mypackage/mypackage-1.0.0.tar.gz#sha256=aaa111""#)
        );
        assert!(
            result
            .contains(r#"href="/pypi/remote-pypi/simple/mypackage/mypackage-1.0.0-py3-none-any.whl#sha256=bbb222""#)
        );
        assert!(result.contains("data-requires-python"));
    ...
    fn test_rewrite_strips_data_dist_info_metadata() {
        ...
        assert!(
            result
            .contains(r#"href="/pypi/pypi-proxy/simple/six/six-1.16.0-py2.py3-none-any.whl#sha256=8abb""#)
        );
...
"#;
        ...
        assert!(
            result
            .contains(r#"href="/pypi/pypi-proxy/simple/six/six-1.17.0-py2.py3-none-any.whl#sha256=4721""#)
        );
        assert!(
            result
            .contains(r#"href="/pypi/pypi-proxy/simple/six/six-1.17.0.tar.gz#sha256=ff70""#)
        );
        assert!(result.contains("data-requires-python"));
        assert!(! result.contains("data-dist-info-metadata"));
        assert!(! result.contains("data-core-metadata"));
    ...
    fn test_rewrite_strips_metadata_attr_before_href() {
        ...
        let result = rewrite_upstream_urls(html, "repo", "pkg");
        assert!(
            result.contains(r#"href="/pypi/repo/simple/pkg/pkg-1.0.whl#sha256=def""#)
        );
        assert!(! result.contains("data-dist-info-metadata"));
    }
    #[test]
    ...
    fn test_rewrite_relative_dotdot_href() {
        ...
        assert!(
            result
            .contains(r#"href="/pypi/pypi-remote/simple/requests/requests-2.31.0.tar.gz#sha256=abc""#)
        );
    }
    #[test]
    fn test_rewrite_root_relative_href() {
        let html = r#"<a href="/packages/ab/cd/six-1.16.0.tar.gz#sha256=abc">six-1.16.0.tar.gz</a>"#;
        let result = rewrite_upstream_urls(html, "repo", "six");
        assert!(
            result
            .contains(r#"href="/pypi/repo/simple/six/six-1.16.0.tar.gz#sha256=abc""#)
        );
    }
    #[test]
    fn test_rewrite_plain_relative_href() {
        ...
        assert!(
            result
            .contains(r#"href="/pypi/devpi-remote/simple/pkg/pkg-1.0.tar.gz#sha256=abc""#)
        );
    }
    #[test]
    fn test_rewrite_mixed_absolute_and_relative_hrefs() {
        ...
        let result = rewrite_upstream_urls(html, "repo", "pkg");
        assert!(
            result.contains(r#"href="/pypi/repo/simple/pkg/pkg-1.0.whl#sha256=aaa""#)
        );
        assert!(
            result.contains(r#"href="/pypi/repo/simple/pkg/pkg-1.0.tar.gz#sha256=bbb""#)
        );
    }
    #[test]
    fn test_find_upstream_url_basic() {
        ...
        let result = find_upstream_url_for_file(
            html,
        ...
        assert_eq!(
            result,
            Some("https://files.pythonhosted.org/packages/d9/5a/six-1.16.0-py2.py3-none-any.whl"
            .to_string())
        );
    }
    #[test]
    ...
    fn test_find_upstream_url_multiple_files() {
        ...
        assert_eq!(
            result,
            Some("https://files.pythonhosted.org/packages/b/six-1.16.0-py2.py3-none-any.whl"
            .to_string())
        );
    }
    #[test]
    fn test_find_upstream_url_with_data_attrs() {
        ...
        assert_eq!(
            result, Some("https://files.pythonhosted.org/packages/numpy-2.0.0.whl"
            .to_string())
        );
    }
    #[test]
...
"#;
        ...
        assert_eq!(
            result,
            Some("https://files.pythonhosted.org/packages/71/39/six-1.16.0-py2.py3-none-any.whl"
            .to_string())
        );
        let result = find_upstream_url_for_file(
            raw_upstream_html,
        ...
        assert_eq!(
            result,
            Some("https://files.pythonhosted.org/packages/94/e7/six-1.16.0.tar.gz"
            .to_string())
        );
    }
    #[test]
...
"#;
    ...
    fn test_find_upstream_url_relative_dotdot_path() {
        ...
        assert_eq!(
            result,
            Some("https://nexus.example.com/repository/pypi/packages/requests-2.31.0.tar.gz"
            .to_string())
        );
    }
    #[test]
    fn test_find_upstream_url_relative_plain_path() {
        ...
        assert_eq!(
            result,
            Some("https://devpi.local/root/pypi/simple/pkg/packages/pkg-1.0.tar.gz"
            .to_string())
        );
    }
    #[test]
    fn test_find_upstream_url_root_relative_path() {
        ...
        assert_eq!(
            result, Some("https://nexus.example.com/packages/ab/cd/six-1.16.0.tar.gz"
            .to_string())
        );
    }
    #[test]
    fn test_find_upstream_url_relative_multiple_dotdot() {
        ...
        assert_eq!(
            result,
            Some("https://artifactory.corp.com/api/pypi/packages/numpy/1.24.0/numpy-1.24.0.tar.gz"
            .to_string())
        );
    }
    #[test]
    fn test_find_upstream_url_relative_prefers_absolute_first() {
        ...
        assert_eq!(
            result, Some("https://files.pythonhosted.org/packages/six-1.16.0.tar.gz"
            .to_string())
        );
    }
    #[test]
...
"#;
        ...
        assert_eq!(
            result,
            Some("https://nexus.example.com/repository/pypi/packages/requests/2.31.0/requests-2.31.0.tar.gz"
            .to_string())
        );
    }
    #[test]
    ...
    fn test_extract_metadata_from_wheel_with_valid_wheel() {
        ...
        writer.start_file("mypackage-1.0.dist-info/METADATA", options).unwrap();
    ...
    fn streaming_result_with(
        ...
        crate::services::proxy_service::StreamingFetchResult {
            body: futures::stream::once(async move { Ok(Bytes::from_static(content)) })
                .boxed(),
            content_type: None,
            content_length: len,
        }
    ...
    fn test_build_streaming_file_response_sdist_content_type() {
        ...
        assert_eq!(resp.status(), StatusCode::OK);
        assert_eq!(resp.headers().get(CONTENT_TYPE).unwrap(), "application/gzip");
    }
    #[test]
    ...
    fn test_build_streaming_file_response_unknown_extension() {
        ...
        assert_eq!(
            resp.headers().get(CONTENT_TYPE).unwrap(), "application/octet-stream"
        );
    }
    #[test]
    ...
    fn test_build_streaming_file_response_content_length() {
        ...
        assert_eq!(
            resp.headers().get(CONTENT_LENGTH).unwrap(), & data.len().to_string()
        );
    }
    ...
    impl crate::storage::StorageBackend for MissingStorage {
        async fn put(&self, key: &str, content: Bytes) -> crate::error::Result<()> {
            self.puts.lock().expect("puts mutex").push((key.to_string(), content));
            Ok(())
        }
    ...
    async fn test_get_remote_cached_or_refetch_refetches_on_missing_storage() {
        ...
        let storage_key = "proxy-cache/pypi-remote/simple/fastapi/fastapi-0.136.1-py3-none-any.whl/__content__";
        ...
        assert_eq!(
            refetch_calls.load(std::sync::atomic::Ordering::SeqCst), 1,
    ...
    async fn test_get_remote_cached_or_refetch_serves_payload_even_if_writeback_fails() {
        ...
        let stream = super::get_remote_cached_or_refetch_stream(
                ...
                move || async move {
                    Ok(Bytes::from_static(b"refetched-when-disk-full"))
                },
            )
            .await
    ...
    async fn test_get_remote_cached_or_refetch_returns_cached_without_refetch() {
        ...
        assert_eq!(
            refetch_calls.load(std::sync::atomic::Ordering::SeqCst), 0,
                ...
                move || {
                    ...
                    async move {
                        refetch_calls_clone
                            .fetch_add(1, std::sync::atomic::Ordering::SeqCst);
                        Err(
                            AppError::BadGateway("upstream timed out".to_string())
                                .into_response(),
                        )
                    }
                },
    ...
    async fn test_serve_file_remote_arm_routes_through_cached_or_refetch_helper() {
        ...
        let storage_key = format!(
            "proxy-cache/{}/simple/{}/{}", fx.repo_key, project, filename
        );
        ...
        assert_eq!(
            response.headers().get(CONTENT_TYPE).expect("Content-Type").to_str()
            .unwrap(), "application/zip",
        );
        assert_eq!(
            response.headers().get(CONTENT_LENGTH).expect("Content-Length").to_str()
            .unwrap(), wheel_bytes.len().to_string(),
        );
        let body_bytes = axum::body::to_bytes(response.into_body(), 1024 * 1024)
            .await
            .expect("read response body");
        assert_eq!(
            & body_bytes[..], wheel_bytes,
    ...
    async fn pypi_upload_queues_sync_tasks_and_preserves_replication_metadata() {
        ...
        assert_eq!(
            status, StatusCode::OK, "PyPI upload must succeed; body: {}",
            String::from_utf8_lossy(& body)
        );
        assert_eq!(
            wait_for_sync_task_count(& fx.pool, fx.repo_id, & artifact_path, 1). await,
            1, "direct PyPI upload must queue exactly one peer sync task"
        );
        ...
        assert_eq!(
            metadata.1["pkg_info"] ["summary"], "PyPI peer replication smoke package"
        );
        ...
        assert_eq!(package.0.as_deref(), Some("PyPI peer replication smoke package"));
        ...
        let replicated_path = format!(
            "{replicated_project}/{replicated_version}/{replicated_filename}"
        );
        let (content_type, body) = pypi_upload_multipart(
            replicated_project,
            replicated_version,
        ...
        assert_eq!(
            sync_task_count(& fx.pool, fx.repo_id, & replicated_path). await, 0,
    ...
    fn test_pypi_content_type_tar_bz2() {
        assert_eq!(pypi_content_type("package-1.0.tar.bz2"), "application/x-bzip2");
    }
    #[test]
    ...
    fn test_pypi_content_type_unknown() {
        assert_eq!(pypi_content_type("package-1.0.egg"), "application/octet-stream");
    }
    #[test]
    fn test_split_url_normal() {
        let result = split_url_base_and_path(
            "https://files.pythonhosted.org/packages/ab/cd/file.whl",
        );
        assert_eq!(
            result, Some(("https://files.pythonhosted.org".to_string(),
    ...
    fn test_split_url_with_port() {
        ...
        assert_eq!(
            result, Some(("http://localhost:8080".to_string(), "api/v1/packages"
            .to_string()))
        );
    }
    #[test]
    ...
    fn test_split_url_with_single_path_segment() {
        ...
        assert_eq!(
            result, Some(("https://example.com".to_string(), "file.whl".to_string()))
        );
    }
    #[test]
    ...
    fn test_build_simple_project_response_html_single_artifact() {
        let artifacts = vec![
            SimpleProjectArtifact { path : "my-package/my_package-1.0.0.tar.gz"
            .to_string(), version : Some("1.0.0".to_string()), size_bytes : 12345,
            checksum_sha256 : "abc123def456".to_string(), metadata : None, upload_time :
            None, }
        ];
        let headers = HeaderMap::new();
        let result = build_simple_project_response(
            &headers,
            "my-virtual",
            "my-package",
            &artifacts,
    ...
    fn test_build_simple_project_response_html_uses_virtual_repo_key() {
        let artifacts = vec![
            SimpleProjectArtifact { path : "packages/my_package-1.0.0.tar.gz"
            .to_string(), version : Some("1.0.0".to_string()), size_bytes : 5000,
            checksum_sha256 : "aaa111bbb222".to_string(), metadata : None, upload_time :
            None, }
        ];
        let headers = HeaderMap::new();
        let result = build_simple_project_response(
        ...
        let body = tokio::runtime::Runtime::new().unwrap().block_on(body_bytes).unwrap();
        let html = String::from_utf8(body.to_vec()).unwrap();
        assert!(
            html
            .contains("/pypi/pypi-virtual/simple/my-package/my_package-1.0.0.tar.gz"),
            "URL should use the virtual repo key, got: {}", html
        );
        assert!(html.contains("sha256=aaa111bbb222"), "URL should include sha256 hash");
        assert!(
    ...
    fn test_build_simple_project_response_html_with_requires_python() {
        ...
        let artifacts = vec![
            SimpleProjectArtifact { path : "pkg-1.0.0-py3-none-any.whl".to_string(),
            version : Some("1.0.0".to_string()), size_bytes : 4000, checksum_sha256 :
            "deadbeef".to_string(), metadata : Some(metadata), upload_time : None, }
        ];
        let headers = HeaderMap::new();
        let result = build_simple_project_response(
            &headers,
    ...
    fn test_build_simple_project_response_html_multiple_artifacts() {
        let artifacts = vec![
            SimpleProjectArtifact { path : "pkg-1.0.0.tar.gz".to_string(), version :
            Some("1.0.0".to_string()), size_bytes : 1000, checksum_sha256 : "aaa"
            .to_string(), metadata : None, upload_time : None, }, SimpleProjectArtifact {
            path : "pkg-2.0.0.tar.gz".to_string(), version : Some("2.0.0".to_string()),
            size_bytes : 2000, checksum_sha256 : "bbb".to_string(), metadata : None,
            upload_time : None, },
        ];
        let headers = HeaderMap::new();
        let result = build_simple_project_response(
    ...
    fn test_build_simple_project_response_json_uses_virtual_repo_key() {
        let artifacts = vec![
            SimpleProjectArtifact { path : "packages/my_package-1.0.0.tar.gz"
            .to_string(), version : Some("1.0.0".to_string()), size_bytes : 5000,
            checksum_sha256 : "abc123".to_string(), metadata : None, upload_time : None,
            }
        ];
        let mut headers = HeaderMap::new();
        headers.insert("accept", "application/vnd.pypi.simple.v1+json".parse().unwrap());
        let result = build_simple_project_response(
            &headers,
            "pypi-virtual",
            "my-package",
    ...
    fn test_build_simple_project_response_json_with_requires_python() {
        let metadata = serde_json::json!(
            { "pkg_info" : { "requires_python" : ">=3.9,<4.0" } }
        );
        let artifacts = vec![
            SimpleProjectArtifact { path : "pkg-1.0.0-py3-none-any.whl".to_string(),
            version : Some("1.0.0".to_string()), size_bytes : 3000, checksum_sha256 :
            "cafe".to_string(), metadata : Some(metadata), upload_time : None, }
        ];
        let mut headers = HeaderMap::new();
        headers.insert("accept", "application/vnd.pypi.simple.v1+json".parse().unwrap());
        let result = build_simple_project_response(
    ...
    fn test_build_simple_project_response_json_emits_upload_time() {
        ...
        let artifacts = vec![
            SimpleProjectArtifact { path : "pkg-1.0.0-py3-none-any.whl".to_string(),
            version : Some("1.0.0".to_string()), size_bytes : 3000, checksum_sha256 :
            "cafe".to_string(), metadata : None, upload_time : Some(upload_time), }
        ];
        let mut headers = HeaderMap::new();
        headers.insert("accept", "application/vnd.pypi.simple.v1+json".parse().unwrap());
        let result = build_simple_project_response(
    ...
    fn test_build_simple_project_response_json_omits_upload_time_when_absent() {
        let artifacts = vec![
            SimpleProjectArtifact { path : "pkg-1.0.0.tar.gz".to_string(), version :
            Some("1.0.0".to_string()), size_bytes : 10, checksum_sha256 : "abc"
            .to_string(), metadata : None, upload_time : None, }
        ];
        let mut headers = HeaderMap::new();
        headers.insert("accept", "application/vnd.pypi.simple.v1+json".parse().unwrap());
        let response = build_simple_project_response(
                &headers,
                "repo",
                "pkg",
                &artifacts,
    ...
    fn test_build_simple_project_response_html_emits_upload_time() {
        ...
        let artifacts = vec![
            SimpleProjectArtifact { path : "pkg-1.0.0.tar.gz".to_string(), version :
            Some("1.0.0".to_string()), size_bytes : 10, checksum_sha256 : "abc"
            .to_string(), metadata : None, upload_time : Some(upload_time), }
        ];
        let response = build_simple_project_response(
                &HeaderMap::new(),
                "repo",
        ...
        assert!(
            ...
            "HTML should include data-upload-time, got: {}", html
        );
    }
    #[test]
    fn test_build_simple_root_response_html() {
        let packages = vec![
            "flask".to_string(), "numpy".to_string(), "requests".to_string(),
        ];
        let headers = HeaderMap::new();
    ...
    fn test_build_simple_root_response_ignores_content_type_for_negotiation() {
        ...
        let response = build_simple_root_response(&headers, "pypi-virtual", &packages)
            .unwrap();
        let ct = response.headers().get(CONTENT_TYPE).unwrap().to_str().unwrap();
        assert_eq!(
    ...
    fn test_normalize_pep503_drops_script_chars() {
        assert_eq!(normalize_pep503("<script>alert(1)</script>"), "scriptalert1script");
        assert_eq!(normalize_pep503("foo\"onerror=alert(1)"), "fooonerroralert1");
    ...
    fn test_build_simple_root_response_escapes_html_in_package_name() {
        let packages = vec![
            "<script>alert('xss')</script>".to_string(), "foo\"onerror=alert(1)\""
            .to_string(), "ampersand&here".to_string(),
        ];
        let headers = HeaderMap::new();
        let response = build_simple_root_response(&headers, "pypi-virtual", &packages)
            .unwrap();
        ...
        assert!(
            ...
            "raw <script> tag MUST NOT appear in rendered HTML: {}", html
        );
        assert!(
            ! html.contains("</script>"),
            "raw </script> tag MUST NOT appear in rendered HTML: {}", html
    ...
    fn test_build_simple_root_response_escapes_html_in_repo_key() {
        let packages = vec!["flask".to_string()];
        let headers = HeaderMap::new();
        let response = build_simple_root_response(
                &headers,
    ...
    fn test_decode_html_entities_minimal_does_not_double_decode() {
        ...
        assert_eq!(decode_html_entities_minimal("&amp;"), "&");
        assert_eq!(decode_html_entities_minimal("&quot;"), "\"");
        assert_eq!(decode_html_entities_minimal("foo &amp; &lt;bar&gt;"), "foo & <bar>");
    ...
    fn test_merge_local_appends_entries_absent_from_remote() {
        ...
        let local = vec![
            SimpleProjectArtifact { path : "pkg/pkg-2.0.0-py3-none-any.whl".to_string(),
            version : Some("2.0.0".to_string()), size_bytes : 4096, checksum_sha256 :
            "ffeeddccbbaa99887766554433221100".to_string(), metadata : None, upload_time
            : None, }
        ];
        let merged = merge_local_into_remote_simple_html(
            &remote,
        ...
        assert!(
            merged
            .contains("/pypi/virt/simple/pkg/pkg-2.0.0-py3-none-any.whl#sha256=ffeeddccbbaa99887766554433221100"),
    ...
    fn test_merge_local_skips_filenames_already_in_remote() {
        ...
        let local = vec![
            SimpleProjectArtifact { path : "pkg/pkg-1.0.0.tar.gz".to_string(), version :
            Some("1.0.0".to_string()), size_bytes : 1024, checksum_sha256 :
    ...
    fn test_merge_emits_data_requires_python_attribute() {
        ...
        let local = vec![
            SimpleProjectArtifact { path : "pkg/pkg-3.0.0.tar.gz".to_string(), version :
            Some("3.0.0".to_string()), size_bytes : 256, checksum_sha256 : "deadbeef"
            .to_string(), metadata : Some(metadata), upload_time : None, }
        ];
        let merged = merge_local_into_remote_simple_html(
            &remote,
            "virt",
        ...
        assert!(
            ...
            "requires_python is HTML-escaped: {}", merged
        );
    }
    #[test]
    fn test_merge_handles_remote_html_without_body_close() {
        ...
        let local = vec![
            SimpleProjectArtifact { path : "pkg/pkg-2.0.0-py3-none-any.whl".to_string(),
            version : Some("2.0.0".to_string()), size_bytes : 1024, checksum_sha256 :
            "cafebabe".to_string(), metadata : None, upload_time : None, }
        ];
        let merged = merge_local_into_remote_simple_html(
            &remote,
            "virt",
    ...
    fn test_parse_simple_root_projects_empty_when_no_anchors() {
        let projects = super::parse_simple_root_projects(
    ...
    const _MIN_CAP: usize = 1024 * 1024;
    const _MAX_CAP: usize = 64 * 1024 * 1024;
    ...
    async fn test_simple_root_remote_proxies_and_caches_upstream_index() {
        ...
        assert_eq!(
            hits.load(Ordering::SeqCst), 1, "upstream hit exactly once on first call"
        );
    ...
    async fn test_pypi_proxy_cache_redirect_none_when_presigned_disabled() {
        ...
        let result = super::pypi_proxy_cache_redirect(
            ...
            .await;
        assert!(
            result.is_none(), "presigned disabled must short-circuit before any redirect"
        );
    }
    #[tokio::test]
    async fn test_pypi_proxy_cache_redirect_none_when_backend_no_redirect_support() {
        ...
        let storage_path = std::env::temp_dir()
            ...
            .into_owned();
        let proxy = tdh::build_proxy_service_with_fs(pool.clone(), &storage_path);
        let state = tdh::build_state_with_proxy_presigned(
            pool.clone(),
            &storage_path,
            proxy.clone(),
        );
    ...
    async fn test_build_streaming_file_response_headers() {
        ...
        let response = build_streaming_file_response(
        ...
        assert_eq!(
            ...
            Some(data_len.to_string().as_str()), "content-length must match"
        );
    }
    #[tokio::test]
    async fn test_proxy_check_cache_streaming_returns_none_on_miss() {
        ...
        let result = super::proxy_helpers::proxy_check_cache_streaming(
            ...
            .await;
        assert!(result.is_none(), "cache miss must yield None, not Some(result)");
    }
}