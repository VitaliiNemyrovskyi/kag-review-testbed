...
pub fn repo_security_router() -> Router<SharedState> {
    Router::new()
        .route("/:key/security", get(get_repo_security).put(update_repo_security))
...
async fn enrich_scans(db: &PgPool, scans: Vec<ScanResult>) -> Result<Vec<ScanResponse>> {
    ...
    let artifact_info: std::collections::HashMap<Uuid, (String, Option<String>)> = if !artifact_ids
        .is_empty()
    {
        sqlx::query!(
...
#[utoipa::path(
    ...
    responses(
        (
            status = 200,
            description = "List of scan configurations",
            body = Vec<ScanConfigResponse>
        ),
    ),
    security(("bearer_auth" = []))
)]
async fn list_scan_configs(
    ...
    let response: Vec<ScanConfigResponse> = configs
        .into_iter()
...
#[utoipa::path(
    ...
    responses(
        ...
        (
            status = 503,
            description = "Scanner service not configured",
            body = crate::api::openapi::ErrorResponse
        ),
    ),
    security(("bearer_auth" = []))
...
async fn trigger_scan(
    ...
    let scanner = state
        ...
        .ok_or_else(|| AppError::ServiceUnavailable(
            "Scanner service not configured".to_string(),
        ))?
        .clone();
    if let Some(artifact_id) = body.artifact_id {
        let prepared = scanner.prepare_artifact_scan(artifact_id, true).await?;
        let scan_result_ids = crate::services::scanner_service::extract_scan_result_ids(
            &prepared,
        );
        let prepared_map = crate::services::scanner_service::prepared_pairs_to_map(
            prepared,
        );
        ...
        return Ok(
            Json(TriggerScanResponse {
                message: crate::services::scanner_service::build_artifact_scan_message(
                    artifact_id,
                ),
                artifacts_queued: 1,
                scan_result_ids,
    ...
    let count: i64 = sqlx::query_scalar!(
        ...
        .map_err(|e| AppError::Database(e.to_string()))?;
    tokio::spawn(async move {
        if let Err(e) = scanner.scan_repository_with_options(repository_id, true).await {
...
#[utoipa::path(
    ...
    context_path = "/api/v1/security",
    tag = "security",
    params(("id" = Uuid, Path, description = "Scan result ID")),
    responses(
...
#[utoipa::path(
    ...
    context_path = "/api/v1/security",
    tag = "security",
    params(("id" = Uuid, Path, description = "Scan result ID"), ListFindingsQuery),
    responses(
        (
            status = 200,
            description = "Paginated list of findings for a scan",
            body = FindingListResponse
        ),
        (
            status = 404,
            description = "Scan not found",
...
#[utoipa::path(
    ...
    responses(
        (
            status = 200,
            description = "Repository security config and score",
            body = RepoSecurityResponse
        ),
        (
            status = 404,
...
#[utoipa::path(
    ...
    responses(
        (
            status = 200,
            description = "Repository security config updated",
            body = ScanConfigResponse
        ),
        (
            status = 404,
            description = "Repository not found",
...
#[utoipa::path(
    ...
    responses(
        (
            status = 200,
            description = "Paginated list of scans for an artifact",
...
#[utoipa::path(
    ...
    context_path = "/api/v1/repositories",
    tag = "security",
    params(("key" = String, Path, description = "Repository key"), ListScansQuery),
    responses(
        (
            status = 200,
            description = "Paginated list of scans for a repository",
            body = ScanListResponse
        ),
        (
            status = 404,
...
mod tests {
    ...
    fn build_repo_scan_message(repository_id: Uuid, count: i64) -> TriggerScanResponse {
        TriggerScanResponse {
            message: format!(
                "Repository scan queued for {} ({} artifacts)", repository_id, count
            ),
            artifacts_queued: count as u32,
            scan_result_ids: Vec::new(),
        }
    ...
    fn test_compute_findings_pagination_capped() {
        let (_page, per_page, _offset) = compute_findings_pagination(
            Some(1),
            Some(1000),
        );
    ...
    fn test_build_artifact_scan_message_nil_uuid() {
        ...
        assert_eq!(resp.artifacts_queued, 1);
        assert!(resp.message.contains("00000000-0000-0000-0000-000000000000"));
    }
    #[test]
    ...
    fn test_trigger_scan_handler_uses_service_unavailable_for_missing_scanner() {
        ...
        let fn_start = src.find(fn_marker).expect("trigger_scan function must exist");
        ...
        let bad_call = format!(
            "{}\"Scanner service not configured\"", internal_variant.as_str()
        );
        assert!(
        ...
        let good_call = format!(
            "{}\"Scanner service not configured\"", good_variant.as_str()
        );
        assert!(
    ...
    fn test_create_policy_request_serde() {
        let json = serde_json::json!(
            { "name" : "strict-policy", "max_severity" : "high", "block_unscanned" :
            true, "block_on_fail" : true, }
        );
    ...
    fn test_create_policy_request_with_all_fields() {
        ...
        let json = serde_json::json!(
            { "name" : "full-policy", "repository_id" : repo_id, "max_severity" :
            "critical", "block_unscanned" : false, "block_on_fail" : false,
            "min_staging_hours" : 24, "max_artifact_age_days" : 90, "require_signature" :
            true, }
        );
        let req: CreatePolicyRequest = serde_json::from_value(json).unwrap();
    ...
    fn test_update_policy_request_serde() {
        let json = serde_json::json!(
            { "name" : "updated-policy", "max_severity" : "medium", "block_unscanned" :
            false, "block_on_fail" : true, "is_enabled" : false, }
        );
    ...
    fn test_update_policy_request_all_fields() {
        let json = serde_json::json!(
            { "name" : "full-update", "max_severity" : "low", "block_unscanned" : true,
            "block_on_fail" : true, "is_enabled" : true, "min_staging_hours" : 48,
            "max_artifact_age_days" : 365, "require_signature" : true, }
        );
        let req: UpdatePolicyRequest = serde_json::from_value(json).unwrap();
    ...
    fn test_list_scans_query_with_values() {
        let id = Uuid::new_v4();
        let json = serde_json::json!(
            { "repository_id" : id, "status" : "completed", "page" : 2, "per_page" : 50,
            }
        );
        let query: ListScansQuery = serde_json::from_value(json).unwrap();
    ...
    fn test_finding_response_serialization() {
        ...
        let resp = FindingResponse {
            ...
            source_url: Some(
                "https://nvd.nist.gov/vuln/detail/CVE-2024-12345".to_string(),
            ),
